...
def isValidWord(word, hand, wordList):
    ...
    try:
        wordList.index(word)
        for letter in word:
            copyHand[letter] = copyHand.get(letter, 0) - 1
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    guessNum = 0
    command = ""
    while calculateHandlen(hand) > 0 and not command == ".":
        ...
        if command == ".":
            ...
            if isValidWord(command, hand, wordList):
                if guessNum == 0 and calculateHandlen(updateHand(hand.copy(), command)) == 0:
                    score = getWordScore(command, n)
                    print(('" {} "'.format(command)), "earned", score, "points. Total: ", score, "points")
                    totalScore += score
                    print()
                    ...
                    score = getWordScore(command, n)
                    totalScore += score
                    print(('" {} "'.format(command)), "earned", score, "points. Total: ", totalScore, "points")
                    hand = updateHand(hand.copy(), command)
                    guessNum += 1
                    print()
                    if calculateHandlen(hand.copy()) == 0:
